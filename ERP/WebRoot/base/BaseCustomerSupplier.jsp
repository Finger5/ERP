...
$(function(){
	...
	$("#customerList").datagrid({//往哪里添加table
		...
		url:'/ERP/BaseCustomerSupplier/GetBaseCustomerSupplierServlet',
...
function delCustomer(code){
	$.messager.confirm("删除提醒","确认删除吗？",function(r){
		if(r){
		//window.location.href='/ERP/customer/deleteCustomerServlet?code='+code;
		
			 $.ajax({
			     url:'/ERP/BaseCustomerSupplier/DeleteBaseCustomerSupplierServlet?code='+code,
		...
		$.messager.confirm("删除确认","确实要删除记录吗？",function(r){
			if(r){//如果点击了确定则执行			    
				for(var i = 0; i < rows.length; i++){
				   ...
				   $.ajax({
				     url:'/ERP/BaseCustomerSupplier/DeleteBaseCustomerSupplierServlet',
				     data:{'code':code},
				     success:function(data){
				     	$("#customerList").datagrid("reload");	
				     }		    
...
function outExcel(){
  window.location.href='/ERP/BaseCustomerSupplier/OutputExcelBaseCustomerSupplierServlet';
}
function freemarker(){
    var code = $("input[name='code']").val();
	
  $.ajax({
    url:'/ERP/BaseCustomerSupplier/OutputWordBaseCustomerSupplierServlet',   
    data:{'code':code},
  })
...
<div id="addCustomer" style="padding:10px;">
	<table border="1" cellspacing="0" style="border-collapse:collapse;">	
    	<form id="myFrm" action="/ERP/BaseCustomerSupplier/UpdateBaseCustomerSupplierServlet" method="post" >
        ...
        <tr>
            ...
            <td>
            	<input type="radio" name="categorycode" value="1" />客户
