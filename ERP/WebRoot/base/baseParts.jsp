...
$(function(){
	...
	$("#partsList").datagrid({    //往哪里添加table
		...
		url:'/ERP/baseParts/SearchBasePartsServlet',
		...
		$.messager.confirm("删除确认","确实要删除记录吗？",function(r){
			if(r){                                 //如果点击了确定则执行			    
				for(var i = 0; i < rows.length; i++){
				   ...
				   $.ajax({
				     url:'/ERP/baseParts/DeleteBasePartsServlet',
...
function delParts(partsCode){
	$.messager.confirm("删除提醒","确实执行删除操作吗？",function(r){
		if(r){
			$.ajax({
			     url:'/ERP/baseParts/DeleteBasePartsServlet?partsCode='+partsCode,
			     success:function(data){			     	
			       $("#partsList").datagrid("reload");
			     }
			 });
		
			//window.location.href="/ERP/baseParts/DeleteBasePartsServlet?partsCode="+partsCode;
		}
	});
}
...
function printParts(idx){
	//window.location.href="/ERP/baseParts/OutWordServlet";
	...
	$.ajax({
	url:'/ERP/baseParts/OutWordBasePartsServlet',
	...
	'salePrice':salePrice,'partsWeight':partsWeight,'isShow':isShow,'remarks':remarks},
	 success:function(data){	
			  ...
			  window.location.href="/ERP/baseParts/DownloadBasePartsServlet?fileName="+fileName;    
			     }
	});
...
//导出excel
function outExcel(){
	window.location.href="/ERP/baseParts/OutExcleBasePartsServlet";
}
//价格调整
...
<body>
<div id="partsTool">
	<form action="/ERP/baseParts/SearchBasePartsServlet" method="post">
        <b>检索条件：</b>
...
<div id="addParts" style="padding:10px;">
	...
	<form id="add" action="/ERP/baseParts/addBasePartsServlet" method="post">
